...
CREATE OR REPLACE PACKAGE currency_pkg IS
    -- Function to retrieve the name of a currency based on its code
    FUNCTION get_currency_name(p_code VARCHAR2) RETURN VARCHAR2;

    -- Procedure to add a new currency
    PROCEDURE add_currency(p_code VARCHAR2, p_symbol VARCHAR2, p_name VARCHAR2);

    -- Procedure to update an existing currency
    PROCEDURE update_currency(p_code VARCHAR2, p_symbol VARCHAR2, p_name VARCHAR2);

    -- Procedure to delete a currency
    PROCEDURE delete_currency(p_code VARCHAR2);

    -- Procedure to retrieve all currencies
    PROCEDURE get_all_currencies;

    -- Function to check if a currency exists
    FUNCTION currency_exists(p_code VARCHAR2) RETURN BOOLEAN;

    -- Procedure to handle dynamic SQL for advanced queries
    PROCEDURE execute_dynamic_sql(p_sql VARCHAR2);

    -- Function to search currencies by name pattern
    FUNCTION search_currencies(p_name_pattern VARCHAR2) RETURN SYS_REFCURSOR;
END currency_pkg;
/

CREATE OR REPLACE PACKAGE BODY currency_pkg IS
    -- Function to retrieve the name of a currency based on its code
    FUNCTION get_currency_name(p_code VARCHAR2) RETURN VARCHAR2 IS
        v_name VARCHAR2(100);
    BEGIN
        SELECT name INTO v_name FROM currencies WHERE code = p_code;
        RETURN v_name;
    EXCEPTION
    ...
    -- Procedure to add a new currency
    PROCEDURE add_currency(p_code VARCHAR2, p_symbol VARCHAR2, p_name VARCHAR2) IS
    BEGIN
        IF NOT currency_exists(p_code) THEN
            INSERT INTO currencies (code, symbol, name) VALUES (p_code, p_symbol, p_name);
        ELSE
            RAISE_APPLICATION_ERROR(-20001, 'Currency already exists.');
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            DBMS_OUTPUT.PUT_LINE('Error in add_currency: ' || SQLERRM);
    END add_currency;

    -- Procedure to update an existing currency
    PROCEDURE update_currency(p_code VARCHAR2, p_symbol VARCHAR2, p_name VARCHAR2) IS
    BEGIN
        IF currency_exists(p_code) THEN
            UPDATE currencies SET symbol = p_symbol, name = p_name WHERE code = p_code;
        ELSE
            RAISE_APPLICATION_ERROR(-20002, 'Currency does not exist.');
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            DBMS_OUTPUT.PUT_LINE('Error in update_currency: ' || SQLERRM);
    END update_currency;

    -- Procedure to delete a currency
    PROCEDURE delete_currency(p_code VARCHAR2) IS
    BEGIN
        IF currency_exists(p_code) THEN
            DELETE FROM currencies WHERE code = p_code;
        ELSE
            RAISE_APPLICATION_ERROR(-20003, 'Currency does not exist.');
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            DBMS_OUTPUT.PUT_LINE('Error in delete_currency: ' || SQLERRM);
    END delete_currency;

    -- Procedure to retrieve all currencies
    PROCEDURE get_all_currencies IS
        CURSOR cur IS SELECT * FROM currencies;
    BEGIN
        FOR rec IN cur LOOP
            DBMS_OUTPUT.PUT_LINE('Code: ' || rec.code || ', Symbol: ' || rec.symbol || ', Name: ' || rec.name);
        END LOOP;
    EXCEPTION
        WHEN OTHERS THEN
            DBMS_OUTPUT.PUT_LINE('Error in get_all_currencies: ' || SQLERRM);
    END get_all_currencies;

    -- Function to check if a currency exists
    FUNCTION currency_exists(p_code VARCHAR2) RETURN BOOLEAN IS
        v_count NUMBER;
    BEGIN
        SELECT COUNT(*) INTO v_count FROM currencies WHERE code = p_code;
        RETURN v_count > 0;
    EXCEPTION
        WHEN OTHERS THEN
            DBMS_OUTPUT.PUT_LINE('Error in currency_exists: ' || SQLERRM);
            RETURN FALSE;
    END currency_exists;
    ...
    BEGIN
        ...
        LOOP
            FETCH v_cursor INTO v_code, v_symbol, v_name;
            EXIT WHEN v_cursor%NOTFOUND;
            DBMS_OUTPUT.PUT_LINE('Code: ' || v_code || ', Symbol: ' || v_symbol || ', Name: ' || v_name);
        END LOOP;
        CLOSE v_cursor;
    EXCEPTION
        WHEN OTHERS THEN
            DBMS_OUTPUT.PUT_LINE('Error in execute_dynamic_sql: ' || SQLERRM);
    ...
    -- Function to search currencies by name pattern
    FUNCTION search_currencies(p_name_pattern VARCHAR2) RETURN SYS_REFCURSOR IS
        v_cursor SYS_REFCURSOR;
    BEGIN
        OPEN v_cursor FOR
        SELECT * FROM currencies WHERE name LIKE p_name_pattern;
        RETURN v_cursor;
    EXCEPTION
        WHEN OTHERS THEN
            DBMS_OUTPUT.PUT_LINE('Error in search_currencies: ' || SQLERRM);
            RETURN NULL;
    END search_currencies;
END currency_pkg;
/

-- Creating the table for storing currency data
...
BEGIN
    -- Add a new currency
    currency_pkg.add_currency('USD', '$', 'United States Dollar');

    -- Update an existing currency
    currency_pkg.update_currency('AFN', '؋', 'Afghan Afghani');

    -- Delete a currency
    currency_pkg.delete_currency('ALL');

    -- Retrieve all currencies
    currency_pkg.get_all_currencies;

    -- Get the name of a currency
    DBMS_OUTPUT.PUT_LINE('Currency name for USD: ' || currency_pkg.get_currency_name('USD'));

    -- Execute dynamic SQL
    currency_pkg.execute_dynamic_sql('SELECT * FROM currencies');

    -- Search currencies by name pattern
    v_cursor := currency_pkg.search_currencies('%Afghan%');
    LOOP
        FETCH v_cursor INTO v_code, v_symbol, v_name;
        EXIT WHEN v_cursor%NOTFOUND;
        DBMS_OUTPUT.PUT_LINE('Code: ' || v_code || ', Symbol: ' || v_symbol || ', Name: ' || v_name);
...
BEGIN
    currency_pkg.add_currency('BAM', 'KM', 'Bosnia and Herzegovina');
END;
/
"""
